...
export const ModernTemplate = ({ data }: { data: ResumeData }) => {
    return (
        <ResumePage className="bg-white text-gray-800 font-sans">
            ...
            <header className="bg-slate-900 text-white p-8 mb-8">
                ...
                <div className="text-xl mt-2 font-light opacity-90 text-gray-300">
                    {data.targetRole || 'Creative Professional'}
                </div>

                <div className="flex flex-wrap gap-4 text-sm mt-6 opacity-80 text-gray-400">
            ...
            <div className="px-8 pb-8 grid grid-cols-12 gap-8">
                ...
                <div className="col-span-4 space-y-8 border-r border-gray-100 pr-4">
                    <section>
                        <h2 className="text-lg font-bold text-slate-900 uppercase mb-3">Links</h2>
                        <div className="flex flex-col gap-2 text-sm">
                            {data.personalInfo.linkedin && (
                                <a href={data.personalInfo.linkedin} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">LinkedIn Profile</a>
                            )}
                            {data.personalInfo.portfolio && (
                                <a href={data.personalInfo.portfolio} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">Portfolio</a>
                            )}
                        </div>
                    </section>
                    ...
                    {data.skills.length > 0 && (
                        <section>
                            <h2 className="text-lg font-bold text-slate-900 uppercase mb-3">Skills</h2>
                    ...
                    {data.education.length > 0 && (
                        <section>
                            <h2 className="text-lg font-bold text-slate-900 uppercase mb-3">Education</h2>
                ...
                <div className="col-span-8 space-y-8">
                    {data.aiSummary && (
                        <section className="bg-gray-50 p-4 rounded-lg border-l-4 border-slate-900">
                            ...
                            <div className="space-y-6">
                                {data.experience.map((exp, i) => (
                                    <div key={i} className="relative pl-4 border-l-2 border-gray-200 break-inside-avoid">
                                        <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-slate-900 ring-4 ring-white"></div>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {data.projects.map((proj, i) => (
                                    <div key={i} className="border rounded-lg p-4 hover:shadow-md transition-shadow break-inside-avoid">
                                        <div className="flex justify-between items-center mb-2">
                                            <h3 className="font-bold text-gray-900">{proj.name}</h3>
                                            {proj.link && (
                                                <a href={proj.link} target="_blank" rel="noreferrer" className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded hover:bg-blue-200">
                                                    Link ↗
                                                </a>
                                            )}
                                        </div>
